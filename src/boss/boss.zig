...
pub const LevelBossData = struct {
    ...
    isBossHit: *const fn (boss: *Boss, hitArea: main.TileRectangle, cutRotation: f32, hitDirection: u8, state: *main.GameState) anyerror!bool,
...
pub fn isBossHit(hitArea: main.TileRectangle, playerBladeRotation: f32, hitDirection: u8, state: *main.GameState) !bool {
    var aBossHit = false;
    var bossIndex: usize = 0;
    while (bossIndex < state.bosses.items.len) {
        const boss = &state.bosses.items[bossIndex];
        const levelBossData = LEVEL_BOSS_DATA[boss.dataIndex];
        if (try levelBossData.isBossHit(boss, hitArea, playerBladeRotation, hitDirection, state)) aBossHit = true;
        if (boss.hp == 0) {
