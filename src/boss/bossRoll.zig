...
fn startBoss(state: *main.GameState) !void {
    ...
    var boss: bossZig.Boss = .{
        .hp = scaledHp,
        .maxHp = scaledHp,
        .imageIndex = bossZig.LEVEL_BOSS_DATA.get(.roll).imageIndex,
        .position = .{ .x = 0, .y = 0 },
        .name = BOSS_NAME,
        .typeData = .{ .roll = .{
            .movePieces = .{
    ...
    if (state.newGamePlus > 0) {
        boss.typeData.roll.moveChargeTime = @divFloor(boss.typeData.roll.moveChargeTime, @as(i32, @intCast(state.newGamePlus + 1)));
    ...
    switch (rollData.state) {
        ...
        .executeMovePiece => {
            if (rollData.nextStateTime <= state.gameTime) {
                try movePieceZig.attackMovePieceCheckPlayerHit(&boss.position, boss.imageIndex, rollData.movePieces[rollData.movePieceIndex], rollData.moveDirection, state);
